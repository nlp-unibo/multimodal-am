...
filepath = Path(os.path.join(ProjectRegistry['local_database'], 'm-arg', 'original data'))
...
filepath_split = Path(os.path.join(ProjectRegistry['local_database'], 'm-arg', 'tokenised text'))
...
filepath_topic = Path(os.path.join(ProjectRegistry['local_database'], 'm-arg', 'preprocessed full dataset'))
df_topic = pd.read_csv(Path(filepath_topic, 'dataset_context_definition.csv'))

# Filepath to load the timestamps
filepath_timestamps = Path(os.path.join(ProjectRegistry['local_database'], 'm-arg', 'timestamps'))
...
filepath_audio = Path(os.path.join(ProjectRegistry['local_database'], 'm-arg', 'split audio'))
filepath_audio_save = Path(os.path.join(ProjectRegistry['local_database'], 'm-arg', 'audio sentences'))

# Each audio file is clipped with a buffer of +- 2 s (custom)
buffer = 2  # In seconds

# For each sub-debate timestamp file
for i in tqdm(debate_id_map):
