...
def save_jsonl(data, output_path, append=False):
    ...
    with open(output_path, mode, encoding='utf-8') as f:
        for line in data:
            ...
            f.write(json_record + '\n')
    Logger.get_logger(__name__).info('Wrote {} records to {}'.format(len(data), output_path))
...
def load_jsonl(input_path) -> list:
    ...
    Logger.get_logger(__name__).info('Loaded {} records from {}'.format(len(data), input_path))
    return data
